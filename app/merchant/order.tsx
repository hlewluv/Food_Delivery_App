  ...
  const [orders, setOrders] = useState<Order[]>([
    {
      ...
      bikerAvatar: 'https://vcdn1-giaitri.vnecdn.net/2025/03/19/jisoo-1-1742349532-1742349698-5135-1742349805.jpg?w=1200&h=0&q=100&dpr=1&fit=crop&s=hGqSXJnPiHy3r6XbGozF2Q',
      time: '10 phút',
      status: 'Đang chuẩn bị',
      items: [
        { item: { id: '1-1-1', name: 'Big Mac', price: '49.000đ', restaurant: 'McDonald\'s', description: 'Bánh burger với 2 miếng thịt bò, sốt đặc biệt, rau sống tươi ngon', image: 29 }, quantity: 1 },
        { item: { id: '1-1-2', name: 'Cheeseburger', price: '29.000đ', restaurant: 'McDonald\'s', description: 'Bánh burger phô mai cổ điển', image: 29 }, quantity: 1 },
      ],
    },
    {
      id: 'GF-002',
      ...
      <View className="bg-gray-50 p-4 rounded-lg mb-4">
        ...
        {order.items.map(({ item, quantity }, index) => (
          <View key={index} className="flex-row justify-between py-2 border-b border-gray-200">
            <Text className="text-base text-gray-600 flex-1">{item.name} x{quantity}</Text>
            ...
            {tabs.map(tab => (
              <View className="p-1" key={tab}>
                <TouchableOpacity
                  ...
                  className={`px-4 py-2 rounded-full ${activeTab === tab ? 'bg-blue-500' : 'bg-gray-100'}`}
                >
                  <Text
        ...
        <View className="bg-white px-3 py-4">
          {filteredOrders.length > 0 ? (
            filteredOrders.map(order => (
              <View key={order.id} className="flex-row justify-between items-start py-3 border-b border-gray-200">
            ...
            <View className="py-4">
              <Text className="text-center text-gray-500">Không có đơn hàng trong danh mục này</Text>
            </View>
          )}
        </View>
      </ScrollView>
