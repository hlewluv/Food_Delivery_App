...
const PaymentMethod = ({ onPaymentMethodChange = (methodId: any) => {}, appliedOffersCount = 0 }) => {
  ...
  return (
    <View className='bg-white rounded-lg px-5 py-4 mt-2 shadow-sm'>
      ...
      <View className='mb-3'>
        <Text className='text-sm font-medium text-gray-500 mb-2'>CHỌN PHƯƠNG THỨC THANH TOÁN</Text>
              ...
              <View className='w-5 h-5 rounded-full border border-gray-300 mr-3 justify-center items-center'>
                <View
                  className={`w-3 h-3 rounded-full ${
                    selectedMethod === method.id ? 'bg-green-500' : 'bg-gray-200'
                  }`}
                />
              </View>
              {method.icon}
